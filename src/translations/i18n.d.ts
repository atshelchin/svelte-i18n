/**
 * Application i18n configuration
 * Auto-generated by @shelchin/svelte-i18n CLI
 *
 * This file handles translation imports and i18n setup
 * Using unified API with type safety for translation keys
 */
import type { I18nPath } from '../types/app-i18n-generated.js';
export declare const i18n: import('../lib/unified.js').TypedI18nInstance<I18nPath>;
export declare function getI18n(): import('../lib/unified.js').TypedI18nInstance<I18nPath>;
export default i18n;
