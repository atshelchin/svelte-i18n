...
export class I18nStore implements I18nInstance {
	...
	loadLanguageSync(locale: string, translations: TranslationSchema | TranslationFile): void {
		...
		console.log(
			`[loadLanguageSync] Validating ${locale}, default locale loaded: ${!!this.translations[this.config.defaultLocale]}`
		);
		if (this.translations[this.config.defaultLocale] && locale !== this.config.defaultLocale) {
			...
			if (errors.length > 0) {
				...
				console.log(
					`[loadLanguageSync] Stored errors for ${locale}, total error locales:`,
		...
		try {
			...
			if (this.translations[this.config.defaultLocale] && locale !== this.config.defaultLocale) {
				...
				if (errors.length > 0) {
					...
					console.log(
						`[Store] Current validationErrors after:`,
						Object.keys(this.validationErrors)
					);
	...
	private revalidateAllTranslations(): void {
		...
		if (!this.translations[defaultLocale]) {
			console.warn(
				`[revalidateAllTranslations] Default locale ${defaultLocale} not loaded, skipping validation`
			);
			return;
		}

		console.log(
			`[revalidateAllTranslations] Revalidating all translations against ${defaultLocale}`
		);
		...
		console.log(
			`[revalidateAllTranslations] Final error locales:`,
			Object.keys(this.validationErrors)
		);
	}
